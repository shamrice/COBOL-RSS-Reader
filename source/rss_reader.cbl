      ...
      *> Purpose: Read RSS feed from command line url.
      *> Tectonics:
      *>     ./build.sh
      *> or: cobc -x rss_reader.cbl rss_parser.cbl cobweb-pipes.cob -o crssr
      *> or: cobc -x -DWIN_NO_POSIX rss_reader.cbl cobweb-pipes.cob -g -debug
      *>*****************************************************************
      ...
      *> No where near perfect... but will at least check if user has some url in their input.
           ...
           if command-arguments = spaces then
               move 'Y' to is-valid-param
               display "No args - TODO : Load parsed feed reader"
           end-if
